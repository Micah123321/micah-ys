...
cheat::GenshinCM::GenshinCM() :
	NFEX(f_AccConfig, u8"帐户配置", u8"数据", u8"通用::多帐号", internal::AccountConfig(), true),
	NFS(f_ShowType,   u8"名称显示类型",         u8"通用::多帐号", ShowType::Pseudo)
{
...
void cheat::GenshinCM::DrawProfileEntryActivities(const std::string& profileName)
{
	...
	if (ImGui::SmallButton(isAccountAttached ? u8"-" : u8"+"))
	{
	...
	if (ImGui::IsItemHovered())
		ImGui::SetTooltip(isAccountAttached ? u8"Dettach" : u8"附加");
...
void cheat::GenshinCM::DrawProfileEntry(const std::string& profileName)
{
	...
	ImGui::TextColored(textColor, u8"%d 账户%s", accountCount, accountCount > 1 ? u8"s" : u8"");
}
...
void cheat::GenshinCM::DrawProfileLine()
{
	...
	if (ImGui::Button(accountAttached ? u8"Deattach" : u8"附加", ImVec2(buttonWidth, 0.0f)))
	{
		if (accountAttached)
...
{
	ConfigWidget(f_ShowType, u8"设置帐户的名称显示类型.");
	auto& pseudos = f_AccConfig.value().pseudos;
	if (pseudos.count(m_CurrentAccount.userID) > 0)
	{
		ImGui::Text(u8"虚拟: %s", pseudos[m_CurrentAccount.userID].c_str()); ImGui::SameLine();
...
void cheat::GenshinCM::OnAccountChanged(uint32_t userID)
{
	...
	ImGuiToast toast(ImGuiToastType_Info, settings.f_NotificationsDelay.value(), u8"账户更新。\nConfig配置文件被更改.");
	toast.set_title(u8"配置multi-account");
