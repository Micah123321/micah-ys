...
namespace cheat::feature
{
	...
	ESP::ESP() : Feature(),
		NF(f_Enabled, u8"透视", u8"透视", false),

		NF(f_DrawBoxMode, u8"绘图模式", u8"透视", DrawMode::Box),
		NF(f_DrawTracerMode, u8"标记模式", u8"透视", DrawTracerMode::Line),
		NF(f_Fill, u8"填满箱/矩形/箭头", u8"透视", false),
		NF(f_FillTransparency, u8"填充透明度", u8"ESP", 0.5f),

		NF(f_ArrowRadius, u8"箭头半径", u8"透视", 100.0f),
		NF(f_OutlineThickness, u8"轮廓厚度", u8"透视", 1.0f),
		NF(f_TracerSize, u8"标记大小", u8"透视", 1.0f),
		NF(f_MiddleScreenTracer, u8"屏幕中示踪剂", u8"透视", false),
		NF(f_DrawDistance, u8"绘制距离", u8"透视", false),
		NF(f_DrawName, u8"绘制名称", u8"透视", false),

		NF(f_FontSize, u8"字体大小", u8"透视", 12.0f),
		NF(f_FontOutline, u8"字形外形", u8"透视", true),
		NF(f_FontOutlineSize, u8"字体轮廓大小", u8"透视", 1.0f),

		NF(f_GlobalFontColor, u8"字体颜色", u8"透视", ImColor(255, 255, 255)),
		NF(f_GlobalBoxColor, u8"箱子颜色", u8"透视", ImColor(255, 255, 255)),
		NF(f_GlobalLineColor, u8"标记颜色", u8"透视", ImColor(255, 255, 255)),
		NF(f_GlobalRectColor, u8"矩形颜色", u8"透视", ImColor(255, 255, 255)),

		NF(f_MinSize, u8"最小实体尺寸", u8"透视", 0.5f),
		NF(f_Range, u8"范围", u8"透视", 100.0f),
		m_Search({})
	{
		InstallFilters();
	...
	{
		static const FeatureGUIInfo info{ u8"", u8"透视", false };
		return info;
	}

	void ESP::DrawMain()
	{
		if (ImGui::BeginGroupPanel(u8"通用", true))
		{
			ConfigWidget(u8"透视启用", f_Enabled, u8"显示过滤后的物体通过障碍物.");
			ConfigWidget(u8"范围 (m)", f_Range, 1.0f, 1.0f, 200.0f);

			ConfigWidget(f_DrawBoxMode, u8"选择画框方式.");
			ConfigWidget(f_DrawTracerMode, u8"选择描图模式.");

			ConfigWidget(f_Fill);
			ConfigWidget(f_FillTransparency, 0.01f, 0.0f, 1.0f, u8"填充部分透明度.");
			ConfigWidget(f_MiddleScreenTracer, u8"从屏幕的中间部分绘制跟踪器.");

			if (f_DrawTracerMode.value() == DrawTracerMode::OffscreenArrows)
			{
				if (ImGui::BeginGroupPanel(u8"箭头示踪剂的选择", true))
				{
					ConfigWidget(f_TracerSize, 0.005f, 0.1f, 10.0f, u8"示踪剂的大小.");
					ConfigWidget(f_ArrowRadius, 0.5f, 50.0f, 300.0f, u8"箭头半径.");
					ConfigWidget(f_OutlineThickness, 0.005f, 0.0f, 10.0f, u8"箭头轮廓厚度.");
				}
				ImGui::EndGroupPanel();
			}

			ImGui::Spacing();
			ConfigWidget(f_DrawName, u8"绘制对象名称.");
			ConfigWidget(f_DrawDistance, u8"绘制物体距离.");

			ImGui::Spacing();
			ConfigWidget(f_FontSize, 1, 1, 100, u8"名称或距离的字体大小.");
			ConfigWidget(u8"## 字体轮廓启用", f_FontOutline); ImGui::SameLine();
			ConfigWidget(u8"字体轮廓", f_FontOutlineSize, 0.001f, 0.0f, 10.0f);

			ImGui::Spacing();
			if (ImGui::BeginGroupPanel(u8"全局颜色", true))
			{
				if (ConfigWidget(f_GlobalFontColor, u8"线条、名称或距离文本字体的颜色."))
					m_FontContrastColor = ImGui::CalcContrastColor(f_GlobalFontColor);

				ConfigWidget(f_GlobalBoxColor, u8"方框字体颜色.");
				ConfigWidget(f_GlobalLineColor, u8"线条字体颜色.");
				ConfigWidget(f_GlobalRectColor, u8"矩形字体的颜色.");
			}
			ImGui::EndGroupPanel();

			ConfigWidget(f_MinSize, 0.05f, 0.1f, 200.0f, u8"在世界中测量的最小实体大小.\n" \
				"有些实体的边界非常小，或者根本没有边界.\n" \
				"此参数帮助过滤掉不满足此条件的实体.");
		}
		ImGui::EndGroupPanel();
		...
		ImGui::InputText(u8"搜索过滤(英文)", &m_Search);
	...
	void ESP::DrawStatus()
	{
		ImGui::Text(u8"透视 [%.01fm|%s|%s|%s|%s|%s]",
			f_Range.value(),
			f_DrawBoxMode.value() == DrawMode::Box ? u8"箱" : f_DrawBoxMode.value() == DrawMode::Rectangle ? u8"矩形" : u8"空",
			f_Fill ? u8"填充" : u8"",
			f_DrawTracerMode.value() == DrawTracerMode::Line ? u8"线" : f_DrawTracerMode.value() == DrawTracerMode::OffscreenArrows ? u8"箭头" : "",
			f_DrawName ? u8"名" : u8"",
			f_DrawDistance ? u8"距" : u8""
		);
	}
	...
	void ESP::AddFilter(const std::string& section, const std::string& name, game::IEntityFilter* filter)
	{
		...
		filters.push_back({ config::CreateField<esp::ESPItem>(name, name, fmt::format(u8"透视::过滤::{}", section), false, newItem), filter });
	}
	//section为物品分类名称 
	...
	void FilterItemSelector(const char* label, ImTextureID image, const config::Field<esp::ESPItem>& field, const ImVec2& size, float icon_size)
	{
		...
		if (ImGui::BeginPopup(u8"选择器"))
		{
			...
			color_changed |= ImGui::ColorPicker4(u8"##选择器", reinterpret_cast<float*>(&field.value().m_Color), picker_flags, &g.ColorPickerRef.x);
			ImGui::EndPopup();
		}
...
#define ADD_FILTER_FIELD(section, name) AddFilter(util::MakeCapital(#section), util::SplitWords(#name), &game::filters::##section##::##name##)
	void ESP::InstallFilters()
	{
		...
		// Bosses. Arranged by u8"type" then alphabetical.
		// Regisvines.
