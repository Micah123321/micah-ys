...
namespace cheat::feature 
{
    ItemTeleportBase::ItemTeleportBase(const std::string& section, const std::string& name) : Feature(),
      NF(f_Key, reinterpret_cast<const char*>(u8"传送到最近"), section, Hotkey()),
      NF(f_ShowInfo, reinterpret_cast<const char*>(u8"展示信息"), section, true),
    ...
    {
		auto desc = util::string_format(u8"按键传送到最近的 %s ", name.c_str());
	...
	bool ItemTeleportBase::NeedInfoDraw() const
{
	...
	void ItemTeleportBase::DrawEntities()
	{
		...
		{
			ImGui::Text(u8"距离 %.03fm", manager.avatar()->distance(entity));
			ImGui::SameLine();
			auto label = util::string_format(u8"传送 ## %p", entity);
