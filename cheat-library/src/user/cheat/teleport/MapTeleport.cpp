...
namespace cheat::feature
{
	...
	MapTeleport::MapTeleport() : Feature(),
		...
		NF(f_Key, u8"传送热键", u8"地图传送", Hotkey('T'))
	{
		// Map touch
	...
	void MapTeleport::DrawMain()
	{
		ConfigWidget(u8"启用",
			f_Enabled,
			u8"启用teleport-to-mark功能.\n" \
			u8"使用方法: \n" \
			u8"\t1. 打开地图.\n" \
			u8"\t2. 按住[传送键]，用左键在地图的任何地方点击.\n" \
			u8"\tok,你已被传送至选定地点.\n" \
			u8"如果传送到一个非常高的位置，传送可能会出错. \n" \
			u8"调整相应的覆盖高度，以帮助避免."
		);
		...
		ConfigWidget(u8"覆盖的高度 (m)", f_DefaultHeight, 1.0F, 200.0F, 800.0F,
			u8"如果传送不能获得目标位置的地面高度，\n它将传送你到这里指定的高度.\n" \
			u8"建议该值至少与山一样高。\n 否则，你可能会从地面掉下去.");

		ConfigWidget(u8"传送热键", f_Key, true,
			u8"在点击目标位置之前按住该键.");
