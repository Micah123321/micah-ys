...
namespace cheat::feature 
{
    GodMode::GodMode() : Feature(),
        NFEX(f_Enabled, u8"上帝模式", u8"上帝模式", u8"玩家", false, false),
        NF(f_AltGodMode, u8"替代上帝模式", u8"玩家", false)
    {
    ...
    {
        static const FeatureGUIInfo info{ u8"", u8"玩家", false };
        return info;
    }
    ...
    void GodMode::DrawMain()
    {
        ConfigWidget(u8"上帝模式", f_Enabled, 
                     ...
                     u8"可能对某些类型的损伤不起作用.");
        ImGui::Indent();
        ConfigWidget(u8"替代上帝模式", f_AltGodMode,
            u8"替代上帝模式，忽略传入的伤害\n" \
            u8"包括环境破坏.");
        ImGui::Unindent();
    }
    ...
    {
        ImGui::Text(u8"上帝模式%s", f_AltGodMode ? u8"+Alt " : u8" ");
    }
	...
	void GodMode::VCHumanoidMove_NotifyLandVelocity_Hook(app::VCHumanoidMove* __this, app::Vector3 velocity, float reachMaxDownVelocityTime, MethodInfo* method)
	{
		...
		// LOG_DEBUG(u8"%s, %f", il2cppi_to_string(velocity).c_str(), reachMaxDownVelocityTime);
