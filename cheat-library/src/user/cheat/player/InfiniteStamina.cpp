...
namespace cheat::feature 
{
    InfiniteStamina::InfiniteStamina() : Feature(),
        ...
        NF(f_PacketReplacement, u8"替换发包", u8"无限体力", false)
    {
    ...
    {
		ConfigWidget(u8"开启", f_Enabled, u8"启用无限耐力选项.");

		ConfigWidget(u8"替换发包", f_PacketReplacement,
			"此模式阻止发送带有耐力成本操作的服务器数据包,\n"
			"例如:游泳、爬山、冲刺等.\n"
			"注意:这可能比标准方法更安全。需要更多的测试.");
    }
    ...
    bool InfiniteStamina::NeedStatusDraw() const
{
    ...
    { 
        ImGui::Text(u8"无限体力 [%s]", f_PacketReplacement ? u8"包" : u8"正常");
    }
