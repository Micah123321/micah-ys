...
namespace cheat::feature
{
	...
	InteractiveMap::InteractiveMap() : Feature(),
		NFEX(f_Enabled, u8"交互地图", u8"交互地图", u8"交互地图", false, false),
		NF(f_SeparatedWindows, u8"分离的窗口", u8"交互地图", true),
		NF(f_CompletionLogShow, u8"完成日志显示", u8"交互地图", false),

		NFS(f_STFixedPoints, u8"固定点", u8"交互地图", SaveAttachType::Global),
		NFS(f_STCustomPoints, u8"自定义点", u8"交互地图", SaveAttachType::Global),
		NFS(f_STCompletedPoints, u8"保存完成点", u8"交互地图", SaveAttachType::Account),

		NF(f_IconSize, u8"图标尺寸", u8"交互地图", 20.0f),
		NF(f_MinimapIconSize, u8"小地图图标大小", u8"交互地图", 14.0f),
		NF(f_DynamicSize, u8"动态尺寸", u8"交互地图", false),
		NF(f_ShowHDIcons, u8"显示高清图标", u8"交互地图", false),

		NF(f_ShowCompleted, u8"显示完成点", u8"交互地图", false),
		NF(f_CompletePointTransparency, u8"完成点透明度", u8"交互地图", 0.5f),
		NF(f_ShowInCompleted, u8"显示在完成点", u8"交互地图", true),
		NF(f_InCompletePointTransparency, u8"在完成点透明度", u8"交互地图", 1.0f),

		NF(f_AutoDetectNewItems, u8"检测到新的项目", u8"交互地图", true),
		NF(f_AutoFixItemPositions, u8"固定 item 位置", u8"交互地图", true),
		NF(f_ObjectCheckOnlyShowed, u8"只显示检测", u8"交互地图", true),
		NF(f_ObjectDetectRange, u8"检测范围", u8"交互地图", 20.0f),
		NF(f_CheckObjectsDelay, u8"检测延迟 (ms)", u8"交互地图", 2000),

		NF(f_AutoDetectGatheredItems, u8"检测收集的物品", u8"交互地图", true),
		NF(f_GatheredItemsDetectRange, u8"检测范围", u8"交互地图", 20.0f),

		NF(f_CompleteNearestPoint, u8"完成最近的点", u8"交互地图", Hotkey()),
		NF(f_RevertLatestCompletion, u8"恢复最新的完成", u8"交互地图", Hotkey()),
		NF(f_CompleteOnlyViewed, u8"只显示完成", u8"交互地图", true),
		NF(f_PointFindRange, u8"点寻找范围", u8"交互地图", 30.0f),

		NFS(f_CustomPointIndex, u8"自定义点指数", u8"交互地图", 1000000),
		NFS(f_LastUserID, u8"最后的用户id", u8"交互地图", 0),
		...
		m_HoveredPoint(nullptr)
	{
		...
		// --Loading user data
		CreateUserDataField(u8"自定义点", f_CustomPointsJson, f_STCustomPoints.value());
		CreateUserDataField(u8"完成点", f_CompletedPointsJson, f_STCompletedPoints.value());
		CreateUserDataField(u8"固定点", f_FixedPointsJson, f_STFixedPoints.value());
	...
	void InteractiveMap::DrawMenu()
	{
		...
		ImGui::BeginGroupPanel(u8"图标视图");
		{
			...
			ConfigWidget(f_DynamicSize, u8"图标的大小将动态地取决于缩放大小。小地图图标不受影响.");
			ConfigWidget(f_ShowHDIcons, u8"切换图标到高清格式.");
		}
		ImGui::EndGroupPanel();

		ImGui::BeginGroupPanel(u8"在完成/图标视图中");
		{
			ConfigWidget(f_ShowCompleted, u8"显示完成点.");
			ConfigWidget(f_CompletePointTransparency, 0.01f, 0.0f, 1.0f, u8"完成点透明度.");
			ConfigWidget(f_ShowInCompleted, u8"显示正在完成点.");
			ConfigWidget(f_InCompletePointTransparency, 0.01f, 0.0f, 1.0f, u8"正在完成点透明度.");
		}
		ImGui::EndGroupPanel();

		ImGui::BeginGroupPanel(u8"项目调整");
		{
			ConfigWidget(f_AutoFixItemPositions, u8"给最近的点固定位置.\n"
				"只有带绿线的项目支持此功能.");

			ConfigWidget(f_AutoDetectNewItems, u8"允许检测交互地图数据中没有的项目。\n"
				"只有带绿线的项目支持此功能。");

			ConfigWidget(f_ObjectCheckOnlyShowed, u8"仅为显示的过滤器检测对象.");

			ConfigWidget(f_ObjectDetectRange, 0.1f, 5.0f, 30.0f,
				"固定位置:只有当项目在这个关于实体位置的范围内找到时，它的位置才会固定。\n"
				"新项目检测:只有当项目没有在实体位置的范围内找到,\n\t 它被检测为新的."
			);

			ConfigWidget(f_CheckObjectsDelay, 10, 100, 100000, u8"调整项目为功耗操作.\n"
				"所以重新扫描会有指定的延迟.");
		}
		ImGui::EndGroupPanel();
		...
		{
			ConfigWidget(f_AutoDetectGatheredItems, u8"启用检测收集的项.\n"
				"它只对启用此功能后将收集的项目起作用.\n"
				"只有带有蓝线的项支持此功能.");

			ConfigWidget(f_GatheredItemsDetectRange, 0.1f, 5.0f, 30.0f,
				"当实体被收集时，在这个范围内找到最近的点.");
		}
		ImGui::EndGroupPanel();

		ImGui::BeginGroupPanel(u8"手动完成");
		{
			ConfigWidget(f_CompleteNearestPoint, true, u8"当按下时，完成最近的头像点.");
			ConfigWidget(f_RevertLatestCompletion, true, u8"按下时，恢复最新的完成操作.");
			ConfigWidget(f_CompleteOnlyViewed, u8"只对可见点执行完整.");
			ConfigWidget(f_PointFindRange, 0.5f, 0.0f, 200.0f, u8"在规定的范围内完成工作。如果为0就等于无限.");
		}
		ImGui::EndGroupPanel();
	}
	...
	void InteractiveMap::DrawFilters(const bool searchFixed)
	{
		...
		HelpMarker(
			"该页后面有项目的过滤器.\n"
			"被激活的物品会出现在迷你/全球地图上。(很明显)\n"
			"每个过滤器都有选项，您可以通过点击过滤器上的右键来访问它.\n"
			"滤镜可以用彩色线条标记,\n"
			"\t它们表明过滤器支持某些特性。(徘徊)\n"
			"现在就这些。快乐的使用 ^)"
		);
		if (searchFixed)
			ImGui::BeginChild(u8"过滤器列表", ImVec2(-1, 0), false, ImGuiWindowFlags_NoBackground);
	...
	void InteractiveMap::DrawFilter(LabelData& label)
	{
		...
		std::string progress_text = fmt::format(u8"{}/{}", label.completedCount, label.points.size());
		...
		if (g.LogEnabled)
			ImGui::LogRenderedText(&label_pos, mixed_value ? u8"[~]" : label.enabled.value() ? u8"[x]" : u8"[ ]");
		...
		if (ImGui::IsItemClicked(ImGuiMouseButton_Right))
			ImGui::OpenPopup(u8"过滤选项");

		if (ImGui::BeginPopup(u8"过滤选项", ImGuiWindowFlags_AlwaysAutoResize))
		{
			if (ImGui::Button(u8"删除进度"))
			{
	...
	void InteractiveMap::CompletePoint(PointData* pointData)
	{
		...
		LOG_WARNING("完成点在 %.0f.", game::EntityManager::instance().avatar()->distance(pointData->levelPosition));
	...
	void InteractiveMap::OnItemGathered(game::Entity* entity)
	{
		...
		for (auto& [labelID, label] : labels)
		{
			...
			if (nearestPoint == nullptr)
			{
				LOG_WARNING(u8"Failed to find uncompleted point for this object.");
				return;
			}
	...
	void InteractiveMap::LoadUserData(const nlohmann::json& data, LoadElementFunc func)
	{
		for (auto& [cSceneID, jLabels] : data.items())
		{
			...
			if (m_ScenesData.count(sceneID) == 0)
			{
				LOG_WARNING(u8"Scene %u don't exist. Maybe map data was updated.", sceneID);
				continue;
			}
			...
			for (auto& [cLabelID, jLabelUserData] : jLabels.items())
			{
				...
				{
					LOG_WARNING(u8"Label %u:%u don't exist. Maybe data was .", sceneID, labelID);
					continue;
				}
	...
	void InteractiveMap::LoadCustomPointData(LabelData* labelData, const nlohmann::json& data)
	{
		...
		{
			LOG_ERROR(u8"Failed to load custom point for label `%u:%s` with position %.1f, %.1f. ID already exist.",
	...
	void InteractiveMap::LoadCompletedPointData(LabelData* labelData, const nlohmann::json& data)
	{
		...
		{
			LOG_WARNING(u8"Point %u don't exist. Maybe data was updated.", pointID);
			return;
		}
		...
		{
			LOG_WARNING(u8"Completed point %u duplicate.", pointID);
			return;
		}
	...
	void InteractiveMap::LoadFixedPointData(LabelData* labelData, const nlohmann::json& data)
	{
		...
		{
			LOG_WARNING(u8"Fixed point %u duplicate.", pointID);
			return;
		}
	...
	void InteractiveMap::SaveFixedPointData(nlohmann::json& jObject, PointData* point)
	{
		...
		jObject.push_back(
			{
				{ u8"point_id", point->id },
				{ u8"x_pos", point->levelPosition.x },
				{ u8"y_pos", point->levelPosition.y }
			}
		);
	}
	...
	void InteractiveMap::CreateUserDataField(const char* name, config::Field<nlohmann::json>& field, SaveAttachType saveType)
	{
		...
		field = config::CreateField<nlohmann::json>(u8"", name, sectionName, saveType != SaveAttachType::Profile, nlohmann::json::object());
	}
	...
	std::string InteractiveMap::GetUserDataFieldSection(SaveAttachType saveType)
	{
		...
		case SaveAttachType::Account:
			return fmt::format(u8"InteractiveMap::accounts::{}", f_LastUserID.value());
...
#undef LOAD_IF
	...
	void InteractiveMap::LoadLabelData(const nlohmann::json& data, uint32_t sceneID, uint32_t labelID)
	{
        ...
        labelEntry.enabled = config::CreateField<bool>(labelEntry.name, labelEntry.clearName,
			fmt::format(u8"InteractiveMap::Filters::Scene{}", sceneID), false, false);
	...
	{
        LoadSceneData(nlohmann::json::parse(ResourceLoader::Load(u8"MapTeyvatData", RT_RCDATA)), 3);
        LoadSceneData(nlohmann::json::parse(ResourceLoader::Load(u8"MapEnkanomiyaData", RT_RCDATA)), 5);
        LoadSceneData(nlohmann::json::parse(ResourceLoader::Load(u8"MapUndegroundMinesData", RT_RCDATA)), 6);
        LoadSceneData(nlohmann::json::parse(ResourceLoader::Load(u8"MapGoldenAppleArchipelagoData", RT_RCDATA)), 9);

        LOG_INFO(u8"Interactive map data loaded successfully.");
    }
	...
	void InteractiveMap::ApplySceneScalling(uint32_t sceneId, const ScallingInput& input)
    {
		...
		LOG_DEBUG(u8"Position scaling for scene %u: scale %0.3f %0.3f, offset %0.3f %0.3f", sceneId, scale.x, scale.y, offset.x, offset.y);
		auto& sceneData = m_ScenesData[sceneId];
...
#undef APPLY_SCENE_OFFSETS
	...
	void InteractiveMap::DrawExternal()
	{
		...
		// Draw windows
		{
			...
			bool menuOpened = ImGui::Begin(u8"Interactive map", nullptr, ImGuiWindowFlags_NoFocusOnAppearing);
			AddWindowRect();
			...
			if (f_SeparatedWindows)
			{
				bool filtersOpened = ImGui::Begin(u8"Filters", nullptr, ImGuiWindowFlags_NoFocusOnAppearing);
				AddWindowRect();
    ...
    void InteractiveMap::DrawPoints()
	{
		...
		for (auto& [labelID, label] : labels)
		{
			...
			auto image = ImageLoader::GetImage(f_ShowHDIcons ? u8"HD" + label.clearName : label.clearName);
			for (auto& [pointID, point] : label.points)
			{
	...
	void InteractiveMap::InitializeEntityFilter(game::IEntityFilter* filter, const std::string& clearName)
	{
		...
		if (labels.size() == 0)
		{
			LOG_DEBUG(u8"Not found filter for item '%s'", clearName.c_str());
			return;
		}
