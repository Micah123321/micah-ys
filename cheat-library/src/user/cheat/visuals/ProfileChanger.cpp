...
namespace cheat::feature
{
    ...
    ProfileChanger::ProfileChanger() : Feature(),
        NF(f_Enabled, u8"自定义配置文件", u8"视觉::配置文件改变", false),
        NF(f_UID, u8"UID", u8"视觉::配置文件改变", false),
        NF(f_UIDWaterMarkPrefix, u8"UID水标前缀", u8"视觉::配置文件改变", false),
        NF(f_UIDsize, u8"UID 尺寸", u8"视觉::配置文件改变", 14),
        NF(f_UIDpos_x, u8"UID  X 坐标", u8"视觉::配置文件改变", static_cast<float>(app::Screen_get_width(nullptr)* 0.96875)),
        NF(f_UIDpos_y, u8"UID  Y 坐标", u8"视觉::配置文件改变", 0),
        NF(f_NickName, u8"昵称", u8"视觉::配置文件改变", false),
        NF(f_Level, u8"级别", u8"视觉::配置文件改变", false),
        NF(f_Exp, u8"经验", u8"视觉::配置文件改变", false),
        NF(f_CurExp, u8"当前经验", u8"视觉::配置文件改变", 1),
        NF(f_MaxExp, u8"最大经验", u8"视觉::配置文件改变", 1),
        NF(f_ExpBar, u8"经验条", u8"视觉::配置文件改变", false),
        NF(f_ExpBarValue, u8"经验条数值", u8"视觉::配置文件改变", 20.0f),
        NF(f_WorldLevel, u8"世界等级", u8"视觉::配置文件改变", false),
        NF(f_Avatar, u8"头像图片", u8"视觉::配置文件改变", false),
        NF(f_Card, u8"卡片图片", u8"视觉::配置文件改变", false),
    ...
    {
        static const FeatureGUIInfo info{ u8"自定义角色资料", u8"视觉", true };
        return info;
    }

    void ProfileChanger::DrawMain()
    {
        ConfigWidget(f_Enabled, u8"自定义角色资料.");
        ConfigWidget(f_UID, u8"更改uid.");
        ConfigWidget(u8"追加 \"UID:\" 前面水印", f_UIDWaterMarkPrefix);
        ConfigWidget(u8"UID 尺寸", f_UIDsize, 0.1, 1, 500.0, u8"Set UID size");
        ...
        ConfigWidget(f_Level, u8"改变等级.");
        ConfigWidget(f_Exp, u8"改变经验.");
        if (f_Exp) {
        ...
        ConfigWidget(f_WorldLevel, u8"改变世界级别.");
        ConfigWidget(f_Avatar, u8"改变头像图片.\n" \
            u8"Note the size of the picture must be: 256x256.\n" \
            u8"Example path: C:\\Avatars.png");

        ConfigWidget(f_Card, u8"改变卡片.\n" \
