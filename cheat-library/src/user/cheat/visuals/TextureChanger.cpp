...
namespace cheat::feature
{
	...
	TextureChanger::TextureChanger() : Feature(),
		NF(f_Enabled, u8"皮肤改变", u8"视觉::皮肤改变", false),
		NF(f_HeadPath, u8"头", u8"视觉::皮肤改变", false),
		NF(f_BodyPath, u8"身体", u8"视觉::皮肤改变", false),
		NF(f_DressPath, u8"衣服", u8"视觉::皮肤改变", false),
	...
	{
		static const FeatureGUIInfo info{ u8"皮肤改变", u8"视觉", true };
		return info;
	}

	void TextureChanger::DrawMain()
	{
		ConfigWidget(f_Enabled, u8"皮肤改变.");
		ImGui::Text(u8"当前的英雄: %s", ActiveHero.c_str());

		ConfigWidget(f_HeadPath, u8"头结构.\n" \
			u8"示例路径: C:\\Head.png");

		ConfigWidget(f_BodyPath, u8"身体 Texture.\n" \
			u8"Example path: C:\\Body.png");

		ConfigWidget(f_DressPath, u8"衣服 Texture.\n" \
