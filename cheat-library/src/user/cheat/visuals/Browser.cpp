...
namespace cheat::feature
{
    ...
    Browser::Browser() : Feature(),
          NFEX(f_Enabled, u8"ä¯ÀÀÆ÷", u8"ä¯ÀÀÆ÷", u8"ÊÓ¾õ", false, false),
    ...
    {
        static const FeatureGUIInfo info{ u8"ä¯ÀÀÆ÷", u8"ÊÓ¾õ", false };
        return info;
    }

    void Browser::DrawMain()
    {
        ConfigWidget(f_Enabled, u8"Create in-game Browser ´´½¨ÓÎÏ·ÄÚä¯ÀÀÆ÷");
        ...
        if (f_Enabled) {
            ...
            if (planeObject != nullptr) {
                if (BrowserComponents == nullptr) {
                    std::string custom_url = f_URL.length() < 2 || f_URL.c_str() == "" ? "https://www.bing.com/" : f_URL.c_str();
