...
namespace cheat::feature 
{
	PacketSniffer::PacketSniffer() : Feature(),
		NF(f_CaptureEnabled, u8"捕捉", u8"网络抓包", false),
		NF(f_ManipulationEnabled, u8"操作", u8"网络抓包", false),
		NF(f_PipeName, u8"命名管道", u8"网络抓包", u8"genshin_packet_pipe")
	...
	{
		static const FeatureGUIInfo info{ u8"网络抓包", u8"设置", true };
		return info;
	}

	void PacketSniffer::DrawMain()
	{
		ImGui::Text(u8"Dev:用于工作需要服务器使用指定名称的命名管道.\n检查'packet-handler'项目，如示例.");
		ConfigWidget(f_PipeName, u8"用于连接的管道名称。更改将适用于下一个游戏启动后.");
		ConfigWidget(f_CaptureEnabled, u8"启用数据包信息捕获并发送到管道，如果它存在.");
		ConfigWidget(f_ManipulationEnabled, u8"通过sniffer启用阻塞和修改报文，会导致网络延迟.");
	}
	...
	bool PacketSniffer::ParseRawPacketData(char* encryptedData, uint32_t length, PipePacketData& dataOut)
	{
		...
		if (magicHead != 0x4567)
		{
			LOG_ERROR(u8"Head magic value for packet is not valid.");
			return false;
		}
		...
		if (magicEnd != 0x89AB)
		{
			LOG_ERROR(u8"End magic value for packet is not valid.");
			return false;
		}
		...
		{
			LOG_ERROR(u8"Packet size is not valid.");
			return false;
		}
