...
namespace cheat
{
	...
	CheatManagerBase::CheatManagerBase() :
		NF(m_SelectedSection, "", reinterpret_cast<const char*>(u8"通用"), 0),
	...
	void CheatManagerBase::DrawMenu()
	{
		...
		if (ImGui::Checkbox(reinterpret_cast<const char*>(u8"锁定键/鼠标"), &m_IsBlockingInput))
		{
			renderer::SetInputLock(this, m_IsBlockingInput);
		}

		if (ImGui::BeginListBox(reinterpret_cast<const char*>(u8"##列表框 2"), ImVec2(175, -FLT_MIN)))
		{
			size_t index = 0;
	...
	{
		if (ImGui::Button(reinterpret_cast<const char*>(u8"添加新配置文件")))
		{
			...
			{
				index++;
				std::string newName = fmt::format(reinterpret_cast<const char*>(u8"配置文件 #{}"), index);
	...
	void CheatManagerBase::DrawProfileEntryActivities(const std::string& profileName)
	{
		...
		if (ImGui::SmallButton(reinterpret_cast<const char*>(u8"重命名")))
			ImGui::OpenRenamePopup(profileName);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip(reinterpret_cast<const char*>(u8"重命名"));
		...
		if (ImGui::SmallButton(reinterpret_cast<const char*>(u8"删除")))
			config::RemoveProfile(profileName);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip(reinterpret_cast<const char*>(u8"删除"));

		ImGui::SameLine();

		if (ImGui::SmallButton(reinterpret_cast<const char*>(u8"拷贝")))
			config::DuplicateProfile(profileName);
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip(reinterpret_cast<const char*>(u8"拷贝配置文件"));
	}
	...
	void CheatManagerBase::DrawProfileConfiguration()
	{
		...
		if (ImGui::BeginTable(reinterpret_cast<const char*>(u8"配置文件列表"), GetProfileTableColumnCount() + 1, flags,
			ImVec2(0.0f, ImGui::GetTextLineHeightWithSpacing() * 10), 0.0f))
		{
			DrawProfileTableHeader();
			ImGui::TableSetupColumn(reinterpret_cast<const char*>(u8"功能"));
	...
	void CheatManagerBase::DrawProfileLine()
	{
		...
		bool buttonPressed = ImGui::Button(reinterpret_cast<const char*>(u8"配置..."));
		...
		if (ImGui::BeginCombo(reinterpret_cast<const char*>(u8"配置文件"), currentProfile.c_str()))
		{
	...
	void CheatManagerBase::DrawStatus() const
	{
		...
		ImGui::Begin(reinterpret_cast<const char*>(u8"查看当前的作弊模式"), nullptr, flags);

		static ImGuiTableFlags tabFlags = ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg;

		if (ImGui::BeginTable(reinterpret_cast<const char*>(u8"功能列表"), 1, tabFlags))
		{
			ImGui::TableSetupColumn(reinterpret_cast<const char*>(u8"开启功能"));
			ImGui::TableHeadersRow();
	...
	void CheatManagerBase::DrawInfo()
	{
		...
		ImGui::Begin(reinterpret_cast<const char*>(u8"信息窗口"), nullptr, flags);
		ImGui::PopStyleColor();

		if (!showAny)
		{
			ImGui::Text(reinterpret_cast<const char*>(u8"没有东西"));
			ImGui::End();
			return;
		}
	...
	void CheatManagerBase::OnRender()
	{
		...
		if (m_IsProfileConfigurationShowed)
		{
			...
			if (ImGui::Begin(reinterpret_cast<const char*>(u8"配置文件配置"), &m_IsProfileConfigurationShowed))
	...
	void CheatManagerBase::CheckToggles(short key) const
	{
		...
		for (auto& field : config::GetFields<config::Toggle<Hotkey>>())
		{
			...
			if (toggle.value.IsPressed(key))
			{
				...
				std::string title = fmt::format("{}: {}", field.friendName(), (toggle ? reinterpret_cast<const char*>(u8"启用") : reinterpret_cast<const char*>(u8"关闭")));
